...
function SHA512(Msg: PByte; Len: PtrInt): TBytes;  //raw 64-byte digest
function SHA512Hex(Msg: PByte; Len: PtrInt): String;
...
function SHA512(Msg: PByte; Len: PtrInt): TBytes;
var H: array[0..7] of QWord;
  buf: TBytes;
  padLen, i: PtrInt;
  bits: QWord;
begin
  H[0]:=QWord($6A09E667F3BCC908);
  H[1]:=QWord($BB67AE8584CAA73B);
  H[2]:=QWord($3C6EF372FE94F82B);
  H[3]:=QWord($A54FF53A5F1D36F1);
  H[4]:=QWord($510E527FADE682D1);
  H[5]:=QWord($9B05688C2B3E6C1F);
  H[6]:=QWord($1F83D9ABFB41BD6B);
  H[7]:=QWord($5BE0CD19137E2179);
  padLen := Len + 1;
  ...
  buf[Len] := $80;
  bits := QWord(Len) shl 3;  // high 64 bits stay 0 (inputs < 2^61 bytes)
